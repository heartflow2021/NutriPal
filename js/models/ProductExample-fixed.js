/**
 * ProductExample-fixed.js
 * 展示如何使用 Product 類的示例文件 (非模組版本)
 */
...
window.loadProducts = async function() {
  try {
    ...
    const products = window.Product.fromArray(data.products);
...
// 示例：基於健康需求篩選產品
window.filterByHealthNeed = function(products, healthNeed) {
  return products.filter(product => product.supportsHealthNeed(healthNeed));
};

// 示例：基於預算範圍篩選產品
window.filterByBudgetRange = function(products, minPrice, maxPrice) {
  return products.filter(product => product.isInPriceRange(minPrice, maxPrice));
};

// 示例：基於成分篩選產品
window.filterByIngredient = function(products, ingredient) {
  return products.filter(product => product.containsIngredient(ingredient));
};

// 示例：尋找相似產品
window.findSimilarProducts = function(products, targetProduct, limit = 5) {
  // 計算所有產品與目標產品的相似度
...
// 示例：產品使用案例
window.showProductUsageExample = async function() {
  // 1. 載入產品資料
  const products = await window.loadProducts();
  if (products.length === 0) return;
  
  // 2. 篩選支持免疫系統的產品
  const immuneProducts = window.filterByHealthNeed(products, '增強免疫力');
  console.log(`找到 ${immuneProducts.length} 個支持免疫系統的產品`);
  
  // 3. 在免疫產品中篩選預算範圍內的產品
  const affordableImmuneProducts = window.filterByBudgetRange(immuneProducts, 0, 500);
  console.log(`找到 ${affordableImmuneProducts.length} 個價格在 0-500 範圍內的免疫產品`);
  
  // 4. 查找含有維生素 C 的產品
  const vitaminCProducts = window.filterByIngredient(products, '維生素 C');
  ...
  if (products.length > 0) {
    ...
    // 6. 顯示與第一個產品相似的產品
    const similarProducts = window.findSimilarProducts(products, firstProduct, 3);
